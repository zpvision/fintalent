...
func publicCatalogHandler(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if kind == "resumes" {
		rows, err = db.QueryContext(r.Context(), `SELECT r.id,u.full_name,COALESCE((SELECT i.value FROM resume_categories rc JOIN dictionary_items i ON i.id=rc.category_id JOIN dictionaries d ON d.id=i.dictionary_id WHERE rc.resume_id=r.id AND d.alias='position' ORDER BY rc.sort_order LIMIT 1),'Финансовый специалист'),COALESCE(c.name,''),COALESCE(r.desired_salary,0),COALESCE(r.work_preferences,''),COALESCE((SELECT string_agg(value,'|||') FROM (SELECT i.value FROM resume_categories rc JOIN dictionary_items i ON i.id=rc.category_id JOIN dictionaries d ON d.id=i.dictionary_id WHERE rc.resume_id=r.id AND d.alias IN ('accounting_areas','software','crm') ORDER BY rc.sort_order LIMIT 6) x),'') FROM resumes r JOIN users u ON u.id=r.user_id LEFT JOIN cities c ON c.id=r.preferred_city_id WHERE r.status='published' AND r.visibility='public' AND r.deleted_at IS NULL AND ($1='' OR u.full_name ILIKE '%'||$1||'%' OR EXISTS(SELECT 1 FROM resume_categories rc JOIN dictionary_items i ON i.id=rc.category_id WHERE rc.resume_id=r.id AND i.value ILIKE '%'||$1||'%')) AND ($2='' OR c.name ILIKE '%'||$2||'%') ORDER BY r.published_at DESC NULLS LAST LIMIT 60`, query, city)
	} else {
	...
	for rows.Next() {
		...
		var tags string
		if kind == "resumes" {
			err = rows.Scan(&x.ID, &x.Name, &x.Title, &x.City, &x.Salary, &x.Description, &tags)
		} else {
