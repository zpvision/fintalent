  ...
  function decorate(select) {
    ...
    function render(query = '') {
      ...
      filtered.forEach(item => {
        ...
        button.onclick = () => { select.value = item.id; input.value = item.name; options.hidden = true; input.setAttribute('aria-expanded', 'false'); select.dispatchEvent(new Event('input', { bubbles: true })); select.dispatchEvent(new Event('change', { bubbles: true })) }
    ...
    input.onfocus = () => { input.value = ''; render(); options.hidden = false; input.setAttribute('aria-expanded', 'true') }
    input.oninput = () => { render(input.value); options.hidden = false }
    input.onkeydown = event => { if (event.key === 'Escape') { options.hidden = true; sync() } }
    select.addEventListener('change', sync)
    sync()
  }
  function decorateAll() { document.querySelectorAll('.ac-service-row select[name="service_id"]').forEach(decorate) }
  document.addEventListener('pointerdown', event => { document.querySelectorAll('.ac-service-search').forEach(picker => { if (!picker.contains(event.target)) { const options = picker.querySelector('.ac-service-search-options'); if (options) options.hidden = true } }) })
