...
function loadScript(src, ready) {
  ...
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error('Не удалось загрузить мастер создания профиля'))
...
export default function ResumeCreatePage() {
  ...
  return <><header className="resume-header"><div className="header-title"><b>Создание профиля</b><small>Быстрое создание</small></div><a className="cancel" href="/">× Отменить</a><button className="help">? Нужна помощь?</button></header>
    <main className="resume-layout"><section className="quiz"><div className="progress-head wizard-progress-head"><div className="wizard-progress-main"><div id="resume-step-track" className="wizard-step-track" aria-label="Прогресс создания профиля" /><div className="wizard-progress-copy"><b id="step-number">Загрузка профиля</b><strong id="step-title">Определяем режим…</strong></div></div></div><div className="question"><h1 id="question-title">Загружаем профиль…</h1><p id="question-description">Подготавливаем подходящие разделы для редактирования.</p><div id="position-options" className="position-options"><div className={`loading${error ? ' error' : ''}`}>{error || 'Загружаем данные…'}</div></div><div className="quiz-note">▷ Результат можно будет отредактировать в любой момент</div><div className="navigation"><button id="back-step" className="back-step hidden">← Назад</button><button id="continue" className="continue">Продолжить <span>→</span></button></div></div></section>
      <aside className="profile-progress"><h2>Ваш профиль</h2><p>Заполненность профиля</p><b><span id="percent">0</span>%</b><div className="bar"><i id="bar" /></div><div className="tip">✣ <span>Мы зададим несколько простых вопросов и создадим сильный профиль</span></div><hr /><h3>Что будет в вашем профиле</h3><ul><li>▷ Опыт работы и навыки</li><li>⌘ Программы и сервисы</li><li>▤ Участки учета</li><li>◫ Отрасли и клиенты</li><li>♙ Обязанности</li><li>♢ Тестирование и подтверждения</li></ul><div className="secure">♢ <span>Ваши данные защищены<br />и не будут переданы третьим лицам</span></div></aside>
    </main><footer className="resume-footer">◷ Среднее время создания профиля: 5–7 минут</footer></>
}
