...
const steps = ['Основная информация', 'Что умеет', 'Где работает', 'Демо и изображения', 'Цена и trial', 'Получение доступа', 'Как это работает', 'Предпросмотр', 'Публикация']
...
function Basic({ value, change, upload }) {
  return <><header><small>ШАГ 1</small><h1>Основная информация</h1><p>Расскажите, какую профессиональную задачу решает ваше решение.</p></header><div className="pm-form-grid"><label className="pm-field wide">Название<input value={value.title} onChange={change('title')} /></label><label className="pm-field wide">Описание<textarea rows="4" value={value.short_description} onChange={change('short_description')} /></label><label className="pm-field wide">Обложка<div className="pm-upload-box">{value.cover_image ? <img src={value.cover_image} alt="Обложка" /> : <span><b>Загрузить обложку</b><small>Рекомендуемый размер 1200 × 675 px · JPG, PNG или WebP до 7 МБ</small></span>}<input type="file" accept="image/*" onChange={(event) => upload(event.target.files[0], 'cover')} /></div></label>{[['tags','Теги через запятую'],['topics','Тематика через запятую'],['audiences','Для кого предназначено']].map(([name,label]) => <label className="pm-field" key={name}>{label}<input value={(value[name] || []).join(', ')} onChange={(event) => change(name)({ target: { value: event.target.value.split(',').map((item) => item.trim()).filter(Boolean) } })} /></label>)}</div></>
}
...
function Media({ value, change, upload, remove }) {
  return <><header><small>ПРОДАЮЩАЯ ДЕМОНСТРАЦИЯ</small><h1>Видео и изображения</h1><p>Добавьте video URL и скриншоты.</p></header><div className="pm-form-grid"><label className="pm-field wide">URL демонстрационного видео<input type="url" value={(value.media.find((item) => item.type === 'VIDEO') || {}).url || ''} onChange={(event) => change(event.target.value)} /></label><label className="pm-field wide">Добавить скриншот<div className="pm-upload-box"><span><b>Загрузить изображение</b><small>Рекомендуемый размер 1440 × 900 px · можно добавить несколько изображений</small></span><input type="file" accept="image/*" onChange={(event) => upload(event.target.files[0], 'media')} /></div></label></div><div className="pm-thumbnails">{value.media.filter((item) => item.type === 'IMAGE').map((item) => <span key={item.url}><img src={item.url} alt="" /><button onClick={() => remove(item)}>×</button></span>)}</div></>
}
...
export default function ProfiMarketCreatePage() {
  usePageStyles(['/static/profimarket.css?v=1', '/static/profimarket-ai-editor.css?v=3']); useDocumentPage({ title: 'Разместить решение — ПрофиМаркет' })
  ...
  let content
  if (step === 0) content = <Basic value={solution} change={change} upload={upload} />
  ...
  else if (step === 4) content = <><header><small>МОНЕТИЗАЦИЯ</small><h1>Цена и пробный период</h1></header><div className="pm-form-grid"><label className="pm-field">Модель оплаты<select value={solution.pricing_type} onChange={change('pricing_type')}><option value="ONE_TIME">Разовая покупка</option><option value="MONTHLY">Цена в месяц</option><option value="YEARLY">Цена в год</option><option value="FREE">Бесплатно</option></select></label>{[['price','Цена, ₽'],['old_price','Старая цена, ₽'],['trial_days','Пробный период, дней']].map(([name,label]) => <label className="pm-field" key={name}>{label}<input type="number" value={solution[name] || ''} onChange={change(name)} /></label>)}</div></>
  ...
  else if (step === 6) content = <HowItWorks items={solution.how_it_works} setItems={(how_it_works) => setSolution({ ...solution, how_it_works })} />
  else if (step === 7) content = <><header><small>ПРОВЕРКА</small><h1>Посмотреть глазами покупателя</h1></header><div className="pm-preview-summary"><div><h2>{solution.title || 'Название ещё не заполнено'}</h2><p>{solution.short_description}</p><a className="pm-add" href={`/profimarket/solution/${solution.id}?preview=1`} target="_blank" rel="noreferrer">Открыть страницу покупателя ↗</a></div></div></>
  else content = <><header><small>ФИНАЛЬНЫЙ ШАГ</small><h1>Всё готово к публикации</h1><p>Проверьте карточку и откройте доступ покупателям.</p></header><div className="pm-ai-publish-visual"><img src="/static/profimarket-ai-publish.png" alt="ИИ-ассистент готов к публикации" /><div><b>Ваш ИИ-ассистент готов к запуску</b><p>Для публикации нужны название, короткое описание и хотя бы одна возможность.</p></div></div><button className="pm-buy pm-ai-publish-button" disabled={!solution.title || !solution.short_description || !solution.ai_features.length || busy} onClick={publish}>Опубликовать решение <b>→</b></button></>
