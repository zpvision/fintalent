...
export default function ProfiMarketCreatePage() {
  ...
  const [params] = useSearchParams(), editID = params.get('id'), [solution, setSolution] = useState(null), [meta, setMeta] = useState({}), [step, setStep] = useState(0), [busy, setBusy] = useState(false), [notice, setNotice] = useState(''), [error, setError] = useState(''), [validation, setValidation] = useState({}), [coverFile, setCoverFile] = useState(null)
  ...
  function validateStep() { const next = {}; if (step === 0) { if (String(solution.title || '').trim().length < 8) next.title = 'Введите не менее 8 символов'; if (!String(solution.short_description || '').trim()) next.short_description = 'Добавьте описание' } if (step === 1 && !solution.ai_features.some((item) => String(item.title || '').trim())) next.features = 'Добавьте хотя бы одну возможность с заголовком'; setValidation(next); return !Object.keys(next).length }
  async function move(next) { if (next > step && !validateStep()) return; if (await save()) { setValidation({}); setStep(next) } }
  ...
  let content
  if (step === 0) content = <Basic value={solution} change={change} changeList={(name, items) => setSolution((value) => ({ ...value, [name]: items }))} upload={selectImage} errors={validation} clearError={(name) => setValidation((current) => ({ ...current, [name]: '' }))} />
  else if (step === 1) content = <Features items={solution.ai_features} error={validation.features} setItems={(items) => { setValidation((current) => ({ ...current, features: '' })); setSolution({ ...solution, ai_features: items }) }} />
  ...
  else content = <><header><small>ФИНАЛЬНЫЙ ШАГ</small><h1>Всё готово к публикации</h1><p>Сначала посмотрите карточку глазами покупателя, затем откройте её для всех.</p></header><div className="pm-ai-publish-visual pm-preview-explained"><span className="pm-preview-hint">ⓘ Предпросмотр</span><span className="pm-preview-tooltip" role="tooltip">Предпросмотр карточки. Настройте данные на предыдущих шагах.</span><img src="/static/profimarket-ai-publish.png" alt="ИИ-ассистент готов к публикации" /><div><b>{solution.title || 'Ваш ИИ-ассистент готов к запуску'}</b><p>{solution.short_description || 'Заполните основную информацию и возможности решения.'}</p></div></div><div className="pm-ai-publish-actions"><a className="secondary pm-preview-action" title="Предпросмотр карточки. Данные настраиваются на предыдущих шагах." href={`/profimarket/solution/${solution.id}?preview=1`} target="_blank" rel="noreferrer">Открыть предпросмотр ↗</a><button className="pm-buy pm-ai-publish-button" disabled={!solution.title || !solution.short_description || !solution.ai_features.length || busy} onClick={publish}>Опубликовать <b>→</b></button></div></>
