...
const editHref=item=>item.type==='REGULATION'?`/profimarket/regulation/edit?id=${item.id}`:item.type==='AI_ASSISTANT'?`/profimarket/create?id=${item.id}`:`/profimarket/product/edit?id=${item.id}`
...
function SolutionRows({ items, reload }) {
  ...
  return <div className="pm-dashboard-list">{items.length ? items.map((item) => <article className="pm-dashboard-row" key={item.id}>
    ...
    <div className="pm-row-actions"><a href={editHref(item)}>Редактировать</a><a href={`/profimarket/solution/${item.id}?preview=1`}>Предпросмотр</a><button onClick={() => action(item)}>{item.status === 'PUBLISHED' ? 'Снять' : 'Удалить'}</button></div>
...
export default function ProfiMarketMyPage() {
  ...
  return <PublicLayout><main className="pm-my-page"><div className="pm-shell">
    <header className="pm-dashboard-head"><div><small>ЛИЧНЫЙ КАБИНЕТ</small><h1>ПрофиМаркет</h1><p>Управляйте продуктами, покупками и заказами.</p></div><Link to="/profimarket/create">＋ Создать продукт</Link></header>
