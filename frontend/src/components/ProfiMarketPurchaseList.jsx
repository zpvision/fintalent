...
import { getMyProfiMarketOrders, getMyProfiMarketPurchases, markMyProfiMarketOrdersRead } from '../api/profimarket'
...
export default function ProfiMarketPurchaseList({ orders = false, onSummary }) {
  ...
  const load=useCallback(()=>{setLoading(true);setError('');(orders?getMyProfiMarketOrders():getMyProfiMarketPurchases()).then(async data=>{setItems(data.items||[]);if(orders){onSummary?.({total_count:data.total_count||0,new_count:data.new_count||0});await markMyProfiMarketOrdersRead();window.dispatchEvent(new CustomEvent('profimarket:orders-read',{detail:{total_count:data.total_count||0,new_count:0}}))}}).catch(e=>setError(e.message)).finally(()=>setLoading(false))},[orders,onSummary])
  useEffect(load,[load])
  ...
  return <div className="profile-market-order-list">{items.map(item=><article className={`profile-market-order${orders&&item.is_new?' is-new':''}`} key={item.id}>
    <header><div><h3>{item.title}</h3><small>{typeLabel(item.type)} · {date(item.created_at)}</small></div><a href={`/profimarket/solution/${encodeURIComponent(item.slug)}`} target="_self">Открыть решение →</a></header>
