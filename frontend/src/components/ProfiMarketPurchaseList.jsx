...
export default function ProfiMarketPurchaseList({ orders = false, onSummary }) {
  ...
  return <div className="profile-market-order-list">{items.map(item=><article className={`profile-market-order${orders&&item.is_new?' is-new':''}`} key={item.id}>
    <header><div><h3>{item.title}</h3><small>{typeLabel(item.type)} · {date(item.created_at)}</small></div><div className="profile-market-order-head-actions">{orders&&item.is_new&&<button type="button" onClick={()=>markRead(item)}>✓ Отметить прочитанным</button>}<a href={`/profimarket/solution/${encodeURIComponent(item.slug)}`} target="_self">Открыть решение →</a></div></header>
