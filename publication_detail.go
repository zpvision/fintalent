...
func publicationRecommendations(w http.ResponseWriter, r *http.Request, id int64) {
	rows, err := db.QueryContext(r.Context(), `SELECT p.id,p.title,p.slug,p.excerpt,p.cover_image,u.full_name,COALESCE(c.name,'') FROM publications p JOIN users u ON u.id=p.author_id LEFT JOIN publication_categories c ON c.id=p.category_id WHERE p.id<>$1 AND p.status='published' AND p.visibility='public' AND p.deleted_at IS NULL AND (NOT u.is_blocked OR u.is_system) ORDER BY (p.category_id=(SELECT category_id FROM publications WHERE id=$1)) DESC,(SELECT COUNT(*) FROM publication_tag_links a JOIN publication_tag_links b ON b.tag_id=a.tag_id WHERE a.publication_id=$1 AND b.publication_id=p.id) DESC,p.published_at DESC LIMIT 3`, id)
	if err != nil {
...
func publicationSitemap(w http.ResponseWriter, r *http.Request) {
	rows, err := db.QueryContext(r.Context(), `SELECT p.slug,p.updated_at FROM publications p JOIN users u ON u.id=p.author_id WHERE p.status='published' AND p.visibility='public' AND p.deleted_at IS NULL AND (NOT u.is_blocked OR u.is_system) ORDER BY p.updated_at DESC`)
	if err != nil {
		http.Error(w, "", 500)
		return
	}
