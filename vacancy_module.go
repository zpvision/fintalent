...
func prepareVacancyModuleDatabase(ctx context.Context) error {
	...
	schema, err = vacancyMigrationFS.ReadFile("migrations/043_resume_birthdate_zodiac.sql")
	if err != nil {
		return err
	}
	_, err = db.ExecContext(ctx, string(schema))
	...
	schema, err = vacancyMigrationFS.ReadFile("migrations/044_test_result_resume_visibility.sql")
	if err != nil {
		return err
	}
	_, err = db.ExecContext(ctx, string(schema))
